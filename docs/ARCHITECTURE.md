...
`main` 是 230×250 的透明 BA仔悬浮窗，默认置顶、不进任务栏；`control` 是通过原生命令创建的完整小管家面板。小猫点击、托盘左键和“打开小猫管家”菜单都会唤醒 `control`。控制面板的关闭按钮只隐藏面板，桌宠继续运行；托盘菜单中的“退出 BA仔”才结束程序。
