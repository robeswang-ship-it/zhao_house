...
- 首次交付 v0.4.3 后，再发布一个 v0.4.4 做真实更新测试。
- Windows 使用圆角小卡片桌宠，而非完全透明的悬浮层；这是为了保证不同 Windows/WebView2 版本都能正常点击与拖动。
- Windows 安装文件名只使用 ASCII 字母、数字与连字符；应用界面、窗口和托盘仍显示“BA仔”。这是为了避免 GitHub 改写中文文件名导致更新下载失败。
